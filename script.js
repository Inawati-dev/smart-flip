...
async function openBook(path, title) {
  ...
  try {
    ...
    showBook();
    preRenderAll(path).catch(() => {});   // background: render sisa halaman (fire-and-forget, guarded)
...
function setPageImg(el, src) {
  if (!src) {
    ...
    el.innerHTML = `<div style="width:100%;height:100%;display:flex;align-items:center;
      ...
      <div style="width:28px;height:28px;border:3px solid var(--terra,#D4A373);border-top-color:transparent;
      ...
      <span style="font-size:11px;font-family:'DM Sans',sans-serif;font-weight:500;
        color:var(--brown2,#6B5D4F)">Memuat…</span>
    </div>`;
    return;
  }
  el.style.background = '';
...
function appendThumb(i, src) {
  ...
  item.innerHTML = src
    ...
    : `<div style="height:72px;width:50px;display:flex;align-items:center;justify-content:center;font-size:10px;color:var(--brown3,#9B8B7A)">${i + 1}</div>`;
